...
function mapHabitToForm(response) {
  return {
    ...
    frecuencia: response.frecuencia || "daily",

    prioridad: response.prioridad || "baja",
    ...
    esCuantificable: response.esCuantificable ?? false,

    cantidadObjetivo: response.cantidadObjetivo ?? "",

    unidadObjetivo: response.unidadObjetivo || "",
  };
}
...
export default function EditHabitPage() {
  ...
  const loadHabit = useCallback(async () => {
    try {
      ...
      setHabit(mapHabitToForm(response));
    } catch (requestError) {
      setError(
        requestError.message ||
          "No pudimos cargar el hábito. Intenta nuevamente.",
      );
    }
  }, [id]);
  ...
  async function handleUpdate(values) {
    try {
      ...
      setFeedback({
        ...
        message:
          requestError.message ||
          "No pudimos actualizar el hábito. Intenta nuevamente.",
      });
    }
  }
      ...
      {habit && (
        <Card>
          ...
          >
            <HabitForm
              cancelHref="/habits"
              initialValues={habit || emptyHabitValues}
              isEditing={true}
