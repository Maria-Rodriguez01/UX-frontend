...
export default function LoginPage() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (token) window.localStorage.setItem('habit-tracker.token', token)

      setFeedback({ severity: 'success', message: 'Sesión iniciada correctamente.' })
      ...
      <Container maxWidth="xs">
        <Paper component="section" sx={{ p: { xs: 3, sm: 4 } }}>
          <Stack alignItems="center" spacing={1} sx={{ mb: 4 }}>
            <TrackChangesRounded color="primary" sx={{ fontSize: 42 }} />
          ...
          <Box component="form" noValidate onSubmit={handleSubmit}>
            <Stack spacing={2.5}>
              ...
              <Button disabled={isSubmitting} size="large" type="submit" variant="contained">
                {isSubmitting ? <CircularProgress color="inherit" size={22} /> : 'Iniciar sesión'}
              </Button>
              <Button disabled={isSubmitting} onClick={() => router.push('/')} variant="text">
                Cancelar
              </Button>
            </Stack>
          </Box>
